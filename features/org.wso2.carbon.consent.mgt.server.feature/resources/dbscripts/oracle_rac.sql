...
CREATE TABLE CM_PURPOSE_VERSION (
  ...
  CONSTRAINT PURPOSE_VERSION_ID_PURPOSE_ID_UNIQUE UNIQUE (ID, PURPOSE_ID),
  PRIMARY KEY (ID)
)
/
CREATE SEQUENCE CM_PURPOSE_VERSION_SEQ START WITH 1 INCREMENT BY 1 CACHE 20 ORDER
/
CREATE OR REPLACE TRIGGER CM_PURPOSE_VERSION_TRIG
  BEFORE INSERT
  ON CM_PURPOSE_VERSION
  REFERENCING NEW AS NEW
  FOR EACH ROW
  BEGIN
    SELECT CM_PURPOSE_VERSION_SEQ.nextval INTO :NEW.ID FROM dual;
  END;
/
CREATE TABLE CM_PURPOSE_VERSION_PII_CAT_ASSOC (
  ID                 INTEGER,
  PURPOSE_VERSION_ID INTEGER NOT NULL,
  CM_PII_CATEGORY_ID INTEGER NOT NULL,
  IS_MANDATORY       INTEGER NOT NULL,
...
/
ALTER TABLE CM_SP_PURPOSE_ASSOC
  ADD CONSTRAINT CM_SP_PURPOSE_ASSOC_fk2 FOREIGN KEY (PURPOSE_VERSION_ID, PURPOSE_ID) REFERENCES CM_PURPOSE_VERSION (ID, PURPOSE_ID)
/
...
ALTER TABLE CM_PURPOSE_VERSION_PII_CAT_ASSOC
  ADD CONSTRAINT CM_PV_PII_CAT_ASSOC_fk0 FOREIGN KEY (PURPOSE_VERSION_ID) REFERENCES CM_PURPOSE_VERSION (ID)
/
ALTER TABLE CM_PURPOSE_VERSION_PII_CAT_ASSOC
