...
public class ConsentsApi  {
    ...
    private ConsentsApiService delegate;
    ...
    @ApiOperation(value = "Create a consent record", notes = "Record user consent for specified purposes and elements. ", response = ConsentResponseDTO.class, tags={ "Consents", })
    @ApiResponses(value = { 
    ...
    @Produces({ "application/json" })
    @ApiOperation(value = "Get a consent record", notes = "Retrieve a specific consent record by receipt ID. ", response = ConsentReceiptDTO.class, tags={ "Consents", })
    @ApiResponses(value = { 
    ...
    @Produces({ "application/json" })
    @ApiOperation(value = "List consent records", notes = "Retrieve consent records with optional filtering by user, service, state, or purpose. ", response = ConsentListResponse.class, tags={ "Consents", })
    @ApiResponses(value = { 
    ...
    public Response consentsList(    @Valid @Size(max=255)@ApiParam(value = "Filter by authorized user ID.")  @QueryParam("subjectUserId") String subjectUserId,     @Valid @Size(max=255)@ApiParam(value = "Filter by service name.")  @QueryParam("service") String service,     @Valid@ApiParam(value = "Filter by consent state.", allowableValues="ACTIVE, REVOKED")  @QueryParam("state") String state,     @Valid@ApiParam(value = "Filter consents by purpose ID.")  @QueryParam("purposeId") Integer purposeId,     @Valid @Min(1) @Max(200)@ApiParam(value = "Maximum number of records to return.", defaultValue="50") @DefaultValue("50")  @QueryParam("limit") Integer limit,     @Valid @Min(0)@ApiParam(value = "Number of records to skip before collecting the response set.", defaultValue="0") @DefaultValue("0")  @QueryParam("offset") Integer offset) {
    ...
    @Produces({ "application/json" })
    @ApiOperation(value = "Revoke a consent record", notes = "Revoke an active consent record. ", response = Void.class, tags={ "Consents" })
    @ApiResponses(value = {
