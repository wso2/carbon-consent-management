...
public class SQLConstants {
    ...
    public static final String LIST_PAGINATED_PURPOSE_MYSQL = "SELECT ID, NAME, DESCRIPTION, PURPOSE_GROUP, " +
                                                              ...
                                                              "WHERE TENANT_ID = ? AND PURPOSE_GROUP LIKE ? ESCAPE '\' AND " +
                                                              "GROUP_TYPE LIKE ? ESCAPE '\' ORDER BY ID ASC LIMIT ? OFFSET ?";
    ...
    public static final String LIST_PAGINATED_PURPOSE_DB2 = "SELECT ID, NAME, DESCRIPTION, PURPOSE_GROUP, GROUP_TYPE," +
                                                            ...
                                                            "TENANT_ID =? AND PURPOSE_GROUP LIKE ? ESCAPE '\' AND GROUP_TYPE " +
                                                            "LIKE ? ESCAPE '\' AND rn BETWEEN ? AND ?";
    ...
    public static final String LIST_PAGINATED_PURPOSE_ORACLE = "SELECT ID, NAME, DESCRIPTION, PURPOSE_GROUP, " +
                                                               ...
                                                               " LIKE ? ESCAPE '\' AND GROUP_TYPE LIKE ? ESCAPE '\' AND rownum <= ?) WHERE " +
                                                               "rnum > ?";
    ...
    public static final String LIST_PAGINATED_PURPOSE_MSSQL = "SELECT ID, NAME, DESCRIPTION, PURPOSE_GROUP," +
                                                              ...
                                                              ".TENANT_ID = ? AND PURPOSE_GROUP LIKE ? ESCAPE '\' AND GROUP_TYPE" +
                                                              " LIKE ? ESCAPE '\' AND P.RowNum BETWEEN ? AND ?";
    ...
    public static final String SEARCH_RECEIPT_SQL = "SELECT R.CONSENT_RECEIPT_ID, R.LANGUAGE, R.PII_PRINCIPAL_ID, R" +
            ...
            "CM_RECEIPT_SP_ASSOC RS ON R.CONSENT_RECEIPT_ID=RS.CONSENT_RECEIPT_ID WHERE PII_PRINCIPAL_ID LIKE ? ESCAPE '\' AND " +
            "PRINCIPAL_TENANT_ID=? AND " +
            "SP_NAME LIKE ? ESCAPE '\' AND SP_TENANT_ID = ? AND STATE LIKE ? ESCAPE '\' ORDER BY ID  ASC LIMIT ? OFFSET ?";
    ...
    public static final String SEARCH_RECEIPT_SQL_DB2 = "SELECT CONSENT_RECEIPT_ID, LANGUAGE, PII_PRINCIPAL_ID, " +
            ...
            ".CONSENT_RECEIPT_ID = RS.CONSENT_RECEIPT_ID WHERE R.PII_PRINCIPAL_ID LIKE ? ESCAPE '\' AND PRINCIPAL_TENANT_ID = ? " +
            "AND RS.SP_NAME LIKE ? ESCAPE '\' AND RS" +
            ".SP_TENANT_ID LIKE ? ESCAPE '\' AND R.STATE LIKE ? ESCAPE '\') AS X WHERE ROWNUMBER BETWEEN ? AND ?";
    ...
    public static final String SEARCH_RECEIPT_SQL_INFORMIX = "SELECT R.CONSENT_RECEIPT_ID, R.LANGUAGE, R" +
            ...
            "LIKE ? ESCAPE '\' AND PRINCIPAL_TENANT_ID =? AND SP_NAME LIKE ? ESCAPE '\' AND SP_TENANT_ID LIKE ? ESCAPE '\' AND STATE LIKE ? ESCAPE '\' ORDER " +
    ...
    public static final String SEARCH_RECEIPT_SQL_ORACLE = "SELECT CONSENT_RECEIPT_ID, LANGUAGE," +
            ...
            "SP_TENANT_ID LIKE ? ESCAPE '\' AND STATE LIKE ? ESCAPE '\' ORDER BY R.CONSENT_RECEIPT_ID ) A WHERE  ROWNUM <= ? ) WHERE  RNUM " +
            "> ?";
    ...
    public static final String SEARCH_RECEIPT_SQL_MSSQL = "SELECT CONSENT_RECEIPT_ID, LANGUAGE, PII_PRINCIPAL_ID, " +
            ...
            ".CONSENT_RECEIPT_ID = RS.CONSENT_RECEIPT_ID WHERE PII_PRINCIPAL_ID LIKE ? ESCAPE '\' AND PRINCIPAL_TENANT_ID = ? " +
            "AND SP_NAME LIKE ? ESCAPE '\' AND " +
            "SP_TENANT_ID LIKE ? ESCAPE '\' AND STATE LIKE ? ESCAPE '\') AS RES WHERE RES.ROWNUM BETWEEN ? AND ?";

    public static final String SEARCH_RECEIPT_SQL_WITHOUT_SP_TENANT = "SELECT R.CONSENT_RECEIPT_ID, R.LANGUAGE, R" +
            ".PII_PRINCIPAL_ID, R.PRINCIPAL_TENANT_ID, R.STATE,RS.SP_DISPLAY_NAME,RS.SP_DESCRIPTION FROM CM_RECEIPT R" +
            " INNER JOIN CM_RECEIPT_SP_ASSOC RS ON R.CONSENT_RECEIPT_ID=RS.CONSENT_RECEIPT_ID WHERE PII_PRINCIPAL_ID " +
            "LIKE ? ESCAPE '\' AND PRINCIPAL_TENANT_ID =? AND SP_NAME LIKE ? ESCAPE '\' AND STATE LIKE ? ESCAPE '\' ORDER BY ID  ASC LIMIT ? OFFSET ?";
    ...
    public static final String SEARCH_RECEIPT_SQL_WITHOUT_SP_TENANT_DB2 = "SELECT CONSENT_RECEIPT_ID, LANGUAGE, " +
            ...
            "LIKE ? ESCAPE '\' AND PRINCIPAL_TENANT_ID=? AND RS.SP_NAME LIKE ? ESCAPE '\' AND R.STATE LIKE ? ESCAPE '\') AS X WHERE ROWNUMBER BETWEEN " +
            "? AND ?";
    ...
    public static final String SEARCH_RECEIPT_SQL_WITHOUT_SP_TENANT_INFORMIX = "SELECT R.CONSENT_RECEIPT_ID, R" +
            ...
            "PII_PRINCIPAL_ID LIKE ? ESCAPE '\' AND PRINCIPAL_TENANT_ID = ? AND SP_NAME LIKE ? ESCAPE '\' AND STATE LIKE ? ESCAPE '\' ORDER BY ID  ASC" +
            " LIMIT ? OFFSET ?";
    ...
    public static final String SEARCH_RECEIPT_SQL_WITHOUT_SP_TENANT_ORACLE = "SELECT CONSENT_RECEIPT_ID, LANGUAGE," +
            ...
            "LIKE ? ESCAPE '\' ORDER BY   R.CONSENT_RECEIPT_ID ) A WHERE  ROWNUM <= ? ) WHERE  RNUM > ?";
    ...
    public static final String SEARCH_RECEIPT_SQL_WITHOUT_SP_TENANT_MSSQL = "SELECT CONSENT_RECEIPT_ID, LANGUAGE, " +
            ...
            ".CONSENT_RECEIPT_ID) AS ROWNUM FROM CM_RECEIPT R INNER JOIN CM_RECEIPT_SP_ASSOC RS ON R" +
            ".CONSENT_RECEIPT_ID = RS.CONSENT_RECEIPT_ID WHERE PII_PRINCIPAL_ID LIKE ? ESCAPE '\' AND PRINCIPAL_TENANT_ID=? AND " +
            "SP_NAME LIKE ? ESCAPE '\' AND " +
            "STATE LIKE ? ESCAPE '\') AS RES WHERE RES.ROWNUM BETWEEN ? AND ?";
    ...
    public static final String SEARCH_RECEIPT_SQL_WITHOUT_PRINCIPLE_TENANT = "SELECT R.CONSENT_RECEIPT_ID, R.LANGUAGE, " +
            ...
            "CM_RECEIPT R INNER JOIN CM_RECEIPT_SP_ASSOC RS ON R.CONSENT_RECEIPT_ID=RS.CONSENT_RECEIPT_ID WHERE " +
            "PII_PRINCIPAL_ID LIKE ? ESCAPE '\' AND SP_NAME LIKE ? ESCAPE '\' AND SP_TENANT_ID = ? AND STATE LIKE ? ESCAPE '\' ORDER BY ID  " +
            "ASC LIMIT ? OFFSET ?";
    ...
    public static final String SEARCH_RECEIPT_SQL_WITHOUT_PRINCIPLE_TENANT_DB2 =
            ...
            ".PII_PRINCIPAL_ID, R.PRINCIPAL_TENANT_ID, RS.SP_DISPLAY_NAME,RS.SP_DESCRIPTION,R.STATE,ROW_NUMBER() OVER" +
            "( ORDER BY R.CONSENT_RECEIPT_ID) AS ROWNUMBER FROM CM_RECEIPT R INNER JOIN CM_RECEIPT_SP_ASSOC RS ON  R" +
            ".CONSENT_RECEIPT_ID = RS.CONSENT_RECEIPT_ID WHERE R.PII_PRINCIPAL_ID LIKE ? ESCAPE '\' AND RS.SP_NAME LIKE ? ESCAPE '\' AND RS" +
            ".SP_TENANT_ID LIKE ? ESCAPE '\' AND R.STATE LIKE ? ESCAPE '\') AS X WHERE ROWNUMBER BETWEEN ? AND ?";
    ...
    public static final String SEARCH_RECEIPT_SQL_WITHOUT_PRINCIPLE_TENANT_INFORMIX =
            ...
            ".PII_PRINCIPAL_ID, R.PRINCIPAL_TENANT_ID, R.STATE,RS.SP_DISPLAY_NAME,RS.SP_DESCRIPTION FROM CM_RECEIPT R" +
            " INNER JOIN CM_RECEIPT_SP_ASSOC RS ON R.CONSENT_RECEIPT_ID=RS.CONSENT_RECEIPT_ID WHERE PII_PRINCIPAL_ID " +
            "LIKE ? ESCAPE '\' AND SP_NAME LIKE ? ESCAPE '\' AND SP_TENANT_ID LIKE ? ESCAPE '\' AND STATE LIKE ? ESCAPE '\' ORDER " +
            "BY ID  ASC LIMIT ? OFFSET ?";
    ...
    public static final String SEARCH_RECEIPT_SQL_WITHOUT_PRINCIPLE_TENANT_ORACLE = "SELECT CONSENT_RECEIPT_ID, LANGUAGE," +
            ...
            ".* FROM( SELECT R.CONSENT_RECEIPT_ID, R.LANGUAGE,R.PII_PRINCIPAL_ID,R.PRINCIPAL_TENANT_ID, R.STATE,RS" +
            ".SP_DISPLAY_NAME,RS.SP_DESCRIPTION FROM CM_RECEIPT R INNER JOIN CM_RECEIPT_SP_ASSOC RS ON R" +
            ".CONSENT_RECEIPT_ID=RS.CONSENT_RECEIPT_ID WHERE PII_PRINCIPAL_ID LIKE ? ESCAPE '\' AND SP_NAME LIKE ? ESCAPE '\' AND " +
            "SP_TENANT_ID LIKE ? ESCAPE '\' AND STATE LIKE ? ESCAPE '\' ORDER BY R.CONSENT_RECEIPT_ID ) A WHERE  ROWNUM <= ? ) WHERE  RNUM " +
            "> ?";
    ...
    public static final String SEARCH_RECEIPT_SQL_WITHOUT_SP_TENANT_AND_PRINCIPLE_TENANT = "SELECT " +
            ...
            ".PRINCIPAL_TENANT_ID, R.STATE,RS.SP_DISPLAY_NAME,RS.SP_DESCRIPTION FROM CM_RECEIPT R INNER JOIN " +
            "CM_RECEIPT_SP_ASSOC RS ON R.CONSENT_RECEIPT_ID=RS.CONSENT_RECEIPT_ID WHERE PII_PRINCIPAL_ID LIKE ? ESCAPE '\' AND " +
            "SP_NAME LIKE ? ESCAPE '\' AND STATE LIKE ? ESCAPE '\' ORDER BY ID  ASC LIMIT ? OFFSET ?";
    ...
    public static final String SEARCH_RECEIPT_SQL_WITHOUT_SP_TENANT_AND_PRINCIPLE_TENANT_DB2 =
            ...
            ".PII_PRINCIPAL_ID, R.PRINCIPAL_TENANT_ID, RS.SP_DISPLAY_NAME,RS.SP_DESCRIPTION,R.STATE,ROW_NUMBER() OVER" +
            "( ORDER BY R.CONSENT_RECEIPT_ID) AS ROWNUMBER FROM CM_RECEIPT R INNER JOIN CM_RECEIPT_SP_ASSOC RS ON  R" +
            ".CONSENT_RECEIPT_ID = RS.CONSENT_RECEIPT_ID WHERE R.PII_PRINCIPAL_ID LIKE ? ESCAPE '\' AND RS.SP_NAME LIKE ? ESCAPE '\' AND " +
            "R.STATE LIKE ? ESCAPE '\') AS X WHERE ROWNUMBER BETWEEN ? AND ?";
    ...
    public static final String SEARCH_RECEIPT_SQL_WITHOUT_SP_TENANT_AND_PRINCIPLE_TENANT_INFORMIX = "SELECT " +
            ...
            "LIKE ? ESCAPE '\' AND SP_NAME LIKE ? ESCAPE '\' AND STATE LIKE ? ESCAPE '\' ORDER BY ID  ASC LIMIT ? OFFSET ?";
    ...
    public static final String SEARCH_RECEIPT_SQL_WITHOUT_SP_TENANT_AND_PRINCIPLE_TENANT_ORACLE =
            ...
            ".SP_DISPLAY_NAME,RS.SP_DESCRIPTION FROM CM_RECEIPT R INNER JOIN CM_RECEIPT_SP_ASSOC RS ON R" +
            ".CONSENT_RECEIPT_ID=RS.CONSENT_RECEIPT_ID WHERE PII_PRINCIPAL_ID LIKE ? ESCAPE '\' AND SP_NAME LIKE ? ESCAPE '\' AND " +
                    "STATE LIKE ? ESCAPE '\' ORDER BY R.CONSENT_RECEIPT_ID ) A WHERE  ROWNUM <= ? ) WHERE  RNUM > ?";
    ...
    public static final String SEARCH_RECEIPT_SQL_WITHOUT_SP_TENANT_AND_PRINCIPLE_TENANT_MSSQL = "SELECT " +
            ...
            ".PRINCIPAL_TENANT_ID, R.STATE, RS.SP_DISPLAY_NAME, RS.SP_DESCRIPTION, ROW_NUMBER() OVER ( ORDER BY R" +
            ".CONSENT_RECEIPT_ID) AS ROWNUM FROM CM_RECEIPT R INNER JOIN CM_RECEIPT_SP_ASSOC RS ON R" +
            ".CONSENT_RECEIPT_ID = RS.CONSENT_RECEIPT_ID WHERE PII_PRINCIPAL_ID LIKE ? ESCAPE '\' AND SP_NAME LIKE ? ESCAPE '\' " +
            "AND STATE LIKE ? ESCAPE '\') AS RES WHERE RES.ROWNUM BETWEEN ? AND ?";
    ...
    public static final String LIST_RECEIPTS_H2_MYSQL_POSTGRES =
            ...
            "  AND r2.PII_PRINCIPAL_ID LIKE ? ESCAPE '\' AND rsa2.SP_NAME LIKE ? ESCAPE '\' AND r2.STATE LIKE ? ESCAPE '\' " +
            "  AND p.UUID LIKE ? ESCAPE '\' AND COALESCE(spa.PURPOSE_VERSION_ID, '') LIKE ? ESCAPE '\' " +
